...
func TestIngesterAppendOutOfOrderAndDuplicate(t *testing.T) {
	...
	err := ing.append(ctx, userID, 0, m, 1, 0, client.API)
	require.NoError(t, err)
	...
	err = ing.append(ctx, userID, 0, m, 0, 0, client.API)
	...
	err = ing.append(ctx, userID, 0, m, 1, 1, client.API)
...
func TestIngesterAppendBlankLabel(t *testing.T) {
	...
	ctx := user.InjectOrgID(context.Background(), userID)
	err := ing.append(ctx, userID, 0, lp, 1, 0, client.API)
	require.NoError(t, err)
